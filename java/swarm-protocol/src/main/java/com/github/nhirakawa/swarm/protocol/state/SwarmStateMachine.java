...
public class SwarmStateMachine extends AbstractExecutionThreadService {
	...
	private final AtomicReference<StateSnapshot> stateSnapshot =
	...
	protected void run() throws Exception {
		while (isRunning()) {
			Optional<StateMachineMessage> maybeStateMachineMessage =
				swarmMessageReceiver.receive(swarmConfig.getProtocolTick());
			...
			var maybeTransition = maybeStateMachineMessage.flatMap(
				stateMachineMessage ->
					switch (stateMachineMessage) {
						case PingAck inboundPingAck -> applyPingAck(inboundPingAck);
						case PingRequest inboundPingRequest -> applyPingRequest(
							inboundPingRequest
						);
						case DiscoveryRequest inboundDiscoveryRequest -> applyDiscoveryRequest(
							inboundDiscoveryRequest
						);
						case DiscoveryResponse inboundDiscoveryResponse -> applyDiscoveryResponse(
	...
	protected String serviceName() {
		return "%s-state-machine".formatted(
			swarmConfig.getLocalAddress().asString()
		);
	}
	...
	private Optional<Transition> applyPingRequest(PingRequest pingRequest) {
