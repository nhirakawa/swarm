...
public class WaitingForPingProxyProtocolStateTest {
	...
	private static final SwarmAddress TARGET = new InMemorySwarmAddress(
		"host-2000"
	);
	private static final SwarmAddress OTHER_1 = new InMemorySwarmAddress(
		"host-3001"
	);
	private static final SwarmAddress OTHER_2 = new InMemorySwarmAddress(
		"host-3002"
	);
	...
	public void setup() {
		...
		protocolState = new WaitingForPingProxyProtocolState(
	...
	public void itTransitionsToWaitingForNextProtocolPeriodAfterProxyAck() {
		...
		Optional<Transition> transition = protocolState.applyPingAck(
			new PingAck(OTHER_1, LOCAL, Optional.of(TARGET), 4L, 0L, List.of())
		);
	...
	public void itTransitionsToWaitingForNextProtocolPeriodAfterProtocolTimeout() {
		...
		WaitingForNextProtocolPeriodProtocolState nextState =
			(WaitingForNextProtocolPeriodProtocolState) transition
				.get()
				.getNextSwarmProtocolState();

		assertThat(nextState.context().protocolPeriodId()).isEqualTo(
	...
	public void itDoesNothingIfAckIsNotFromProxy() {
		Optional<Transition> transition = protocolState.applyPingAck(
			new PingAck(OTHER_2, LOCAL, Optional.of(TARGET), 4L, 0L, List.of())
		);
	...
	public void itDoesNothingIfAckIsNotForTarget() {
		Optional<Transition> transition = protocolState.applyPingAck(
			new PingAck(OTHER_1, LOCAL, Optional.of(OTHER_2), 4L, 0L, List.of())
		);
