...
public class InitializingProtocolState extends SwarmProtocolState {
	...
	public Optional<Transition> applyTick() {
		...
		LOG.debug(
			"Tick fired - registry size is {}",
			context().memberRegistry().size()
		);

		if (context().memberRegistry().size() > 0) {
			// Transition to normal operation
			SwarmProtocolState nextState =
				new WaitingForNextProtocolPeriodProtocolState(context().next());
		...
		LOG.debug("Broadcasting discovery request (attempt {})", attemptNumber + 1);
