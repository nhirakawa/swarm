...
public class GaussianLatencyDistribution implements LatencyDistribution {
  ...
  public GaussianLatencyDistribution(Duration mean, Duration standardDeviation) {
    Preconditions.checkArgument(!mean.isNegative(), "mean must be non-negative");
    Preconditions.checkArgument(
      !standardDeviation.isNegative() && !standardDeviation.isZero(),
      "standardDeviation must be positive"
    );
    this.distribution = new NormalDistribution(mean.toNanos(), standardDeviation.toNanos());
  }
