...
public sealed interface MemberStatus
	permits MemberStatus.Alive, MemberStatus.Suspected, MemberStatus.Confirmed
{
	...
	record Alive(SwarmAddress address, long incarnation) implements MemberStatus {
		...
		public MemberStatus merge(MemberStatus memberStatus) {
			return switch (memberStatus) {
				case Alive(SwarmAddress ignored, long otherIncarnation) -> new Alive(
					address,
					Math.max(incarnation, otherIncarnation)
				);
	...
	record Suspected(
		...
		public MemberStatus merge(MemberStatus memberStatus) {
			return switch (memberStatus) {
				...
				case Suspected(
					SwarmAddress ignored,
					long otherIncarnation
				) -> new Suspected(address, Math.max(incarnation, otherIncarnation));
				case Confirmed(SwarmAddress ignored1, long ignored2) -> memberStatus;
			};
		}
	...
	record Confirmed(
		...
		public MemberStatus merge(MemberStatus memberStatus) {
			if (
				memberStatus instanceof
					Confirmed(SwarmAddress ignored, long otherIncarnation)
			) {
